...
interface OtoActiveCampaignFormProps {
  onRegistrationSuccess?: (registration: WebinarRegistration) => void;
  className?: string;
}

export function OtoActiveCampaignForm({ onRegistrationSuccess, className = "" }: OtoActiveCampaignFormProps) {
        ...
        for (const mutation of mutations) {
          if (mutation.type === 'childList') {
            ...
            if (formElements && formElements.length > 0) {
              ...
              if (titleElement) {
                titleElement.textContent = 'OFFERTA SPECIALE';
              }
              
              // Modifichiamo la descrizione
              const descriptionElement = formContainerRef.current?.querySelector('._form_3 ._html-code p');
              if (descriptionElement) {
                descriptionElement.innerHTML = 'Compila il form per partecipare all\'offerta esclusiva dello Smart Revolution Sprint a soli €37 (invece di €397).';
              }
              ...
              if (submitButton) {
                submitButton.textContent = 'ACQUISTA ORA';
              }
